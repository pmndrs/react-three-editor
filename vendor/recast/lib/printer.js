  ...
  switch (n.type) {
    ...
    case "ReturnStatement": {
      ...
      if (n.argument) {
        ...
        if (
          ...
          parts.push(argLines.indent(options.tabWidth));
        } else {
          parts.push(" ", argLines);
        }
      }
    ...
    case "VariableDeclarator":
      return n.init
        ? lines_1
            .fromString(" = ")
            .join([path.call(print, "id"), path.call(print, "init")])
        : path.call(print, "id");
    case "WithStatement":
      return lines_1.concat([
        "with (",
    ...
    case "DeclareClass":
      ...
      if (n.extends && n.extends.length > 0) {
        ...
        parts.push(
          " extends ",
          lines_1.fromString(", ").join(path.map(print, "extends")),
        );
      }
      if (n["implements"] && n["implements"].length > 0) {
        parts.push(
          " implements ",
          lines_1.fromString(", ").join(path.map(print, "implements")),
        );
      }
      parts.push(" ", path.call(print, "body"));
...
function printExportDeclaration(path, options, print) {
  ...
  if (decl.declaration) {
    ...
    if (
      ...
      parts.push(
        shouldPrintSpaces ? "{ " : "{",
        lines_1.fromString(", ").join(path.map(print, "specifiers")),
        shouldPrintSpaces ? " }" : "}",
      );
    }
    if (decl.source) {
